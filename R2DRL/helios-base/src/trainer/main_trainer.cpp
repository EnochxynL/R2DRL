...
SampleTrainer::actionImpl()
{
    ...
    // 3) 如果你暂时还想只在 PlayOn 里处理，就保留这个。
    //    但以后如果想让 OP_PLAY_ON / RESET 在非 PlayOn 也能工作，应该删掉这个 gate。
    ...
    if (a == 0 && b == 1) {
        ...
        std::cerr << "[trainer][IPC] REQUEST received.\n";
        a = 1;
        b = 0;
    }
    ...
    if (a == 1 && b == 0) {
        trainer_acquire_fence();  // 再做一次也没问题，稳一点
    ...
    // 8) 其他非法状态
    std::cerr << "[trainer][ERR] invalid flags state A=" << int(a)
              << " B=" << int(b) << "\n";
}
